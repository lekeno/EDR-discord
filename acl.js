...
module.exports = {
  ...
  authorizeGuild: function(guildID) {
    if (
      !guildID ||
      !/^\d+$/.test(guildID) ||
      module.exports.authorizedGuild(guildID)
    )
      return false;
    if (module.exports.blockedGuild(guildID)) {
      ...
      if (index > -1) {
        ...
        fs.writeFileSync(
          process.env.BLOCKED_GUILDS,
          JSON.stringify(BLOCKED_GUILDS),
          err => {
            if (err) {
              console.error(err);
              return false;
    ...
    fs.writeFileSync(
      process.env.AUTHORIZED_GUILDS,
      JSON.stringify(AUTHORIZED_GUILDS),
      err => {
        if (err) {
          console.error(err);
          return false;
  ...
  blockGuild: function(guildID) {
    if (
      !guildID ||
      !/^\d+$/.test(guildID) ||
      module.exports.blockedGuild(guildID)
    )
      return false;
  ...
  blockUser: function(userID) {
    if (!userID || !/^\d+$/.test(userID) || module.exports.blockedUser(userID))
      return false;
    BLOCKED_USERS.push(userID);
    fs.writeFileSync(
      process.env.BLOCKED_USERS,
      JSON.stringify(BLOCKED_USERS),
      err => {
        if (err) {
          console.error(err);
          return false;
...
const AUTHORIZED_GUILDS = utils.requireIfExists(
  process.env.AUTHORIZED_GUILDS,
  []
);
